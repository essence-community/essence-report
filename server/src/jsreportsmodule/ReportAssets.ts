...
import * as ReplaceAsync from "async-replace";
// @ts-ignore
...
async function readAsset(reporter, definition, id, name, encoding, req) {
    ...
    if (encoding === "dataURI") {
        escape = (val, nameUri) => {
            const type = mime.getType(nameUri);
    ...
    if (id) {
        ...
        if (pathParts.length === 0) {
            throw reporter.createError(
                "Invalid asset path, path should target something",
                {
                    statusCode: 400,
                    weak: true,
                },
        ...
        const result = await reporter.folders.resolveEntityFromPath(
            name,
            "assets",
            req,
        );
        ...
        if (assets.length > 1) {
            throw reporter.createError(
                `Duplicated assets found for ${assetName}`,
                {
                    statusCode: 400,
                    weak: true,
                },
        ...
        if (!asset) {
            if (
                ...
                name.indexOf(req.template.assetPath) === -1
            ) {
            ...
            try {
                ...
                throw new Error(
                    `Asset ${name} not found in the store and also not on the disk: ` +
                        e.message,
                );
            }
            ...
            return {
                content: escape(
                    Buffer.from(file.content || "").toString(encoding),
                    file.filename,
                ),
    ...
    if (encoding === "link") {
        if (asset.link && !allowAssetsLinkedToFiles) {
            throw reporter.createError(
                `Can't not read asset "${name}" from .link path "${asset.link}" when "allowAssetsLinkedToFiles" option is false`,
                {
                    statusCode: 400,
                    weak: true,
                },
...
async function evaluateAssets(reporter, definition, stringToReplace, req) {
    ...
    function convert(str, p1, offset, s, done) {
        const assetName =
            p1.indexOf(" @") !== -1 ? p1.substring(0, p1.indexOf(" @")) : p1;
        ...
        if (p1.indexOf(" @") !== -1) {
            ...
            if (paramRaw.split("=").length !== 2) {
                throw new Error(
                    "Wrong asset param specification, should be {#asset name @encoding=base64}",
                );
            }
            ...
            if (
                ...
                throw new Error(
                    "Unsupported asset encoding param value " +
                        paramValue +
                        ", supported values are base64, utf8, link, dataURI and string",
                );
            }
            ...
            if (
                ...
                throw new Error(
                    "Asset encoded as dataURI needs to have file extension jpeg|jpg|gif|png|svg|woff|tff|otf|woff2|eot",
                );
            }
        ...
        readAsset(
            ...
            definition,
            null,
            req.template.assetPath
                ? `${req.template.assetPath}/${assetName}`
                : assetName,
            encoding,
            req,
        )
            .then((resAsset) => {
                ...
                done(null, resAsset.content);
            })
            .catch(done);
    }

    const result: string = await new Promise((resolve, reject) => {
        ReplaceAsync(stringToReplace, test, convert, (err, data) => {
            if (err) {
                return reject(err);
            }
            resolve(data);
    ...
    if (replacedAssets.length) {
        reporter.logger.debug(
            "Replaced assets " + JSON.stringify(replacedAssets),
            req,
        );
    }
...
export function isAssetPathValid(allowedFiles, link, absolutePath) {
    return (
        allowedFiles != null &&
        (minimatch(absolutePath, allowedFiles) ||
            minimatch(absolutePath.replace("/", "\\"), allowedFiles) ||
            minimatch(link, allowedFiles) ||
            minimatch(link.replace("/", "\\"), allowedFiles))
    );
}

function linkPath(reporter, definition, link) {
    const result = path.isAbsolute(link)
        ? link
        : path.join(reporter.options.rootDirectory, link);
    ...
    if (!isAssetPathValid(definition.options.allowedFiles, link, result)) {
        throw reporter.createError(
            `Request to file ${result} denied. Please allow it by setting config { "extensions": { "assets": { "allowedFiles": ${
                extension !== "" ? `"**/foo${extension}"` : '"**/*.*"'
            } } } }${
                extension !== ""
                    ? ' or the more general { "extensions": { "assets": { "allowedFiles": "**/*.*" } } }'
                    : ""
            }`,
            {
                weak: true,
...
async function readFile(reporter, definition, link) {
    ...
    try {
        const content: Buffer = await new Promise((resolve, reject) => {
            fs.readFile(pathToLinkedFile, (err, data) => {
                if (err) {
                    return reject(err);
                }
                resolve(data);
        ...
        const stat: fs.Stats = await new Promise((resolve, reject) => {
            fs.stat(pathToLinkedFile, (err, data) => {
                if (err) {
                    return reject(err);
                }
                resolve(data);
        ...
        throw reporter.createError(
            `Unable to find or read file ${pathToLinkedFile}`,
            {
                weak: true,
                original: e,
            },
...
function resolveAssetLink(reporter, definition, req, assetName) {
    if (definition.options.rootUrlForLinks) {
        return url.resolve(
            definition.options.rootUrlForLinks,
            "assets/content/" + assetName,
        );
    }
...
export function InitReportAssets(reporter, definition) {
    ...
    reporter.addRequestContextMetaConfig("evaluateAssetsCounter", {
        sandboxHidden: true,
    });

    if (
        reporter.options.allowLocalFilesAccess === true &&
        !definition.options.allowedFiles &&
        definition.options.searchOnDiskIfNotFoundInStore == null
    ) {
        definition.options.allowedFiles = "**/*.*";
        definition.options.searchOnDiskIfNotFoundInStore = true;
    }
    ...
    reporter.on("express-configure", (app) => {
            ...
            fn()
                ...
                .catch((e) => {
                    reporter.logger.warn(
                        "Unable to get asset content  " + e.stack,
                    );
        ...
        app.get("/assets/office/:id/content", async (req, res) => {
            try {
                ...
                await response(
                    {
                        previewOptions:
                            reporter.options.office != null &&
                            reporter.options.office.preview != null
                                ? reporter.options.office.preview
                                : {},
                        officeDocumentType: path
                ...
                reporter.logger.warn(
                    `Unable to get office asset content ${e.stack}`,
                );
                res.status(500).end(e.message);
            }
        });
        ...
        app.get("/assets/link/:path*", (req, res) => {
            ...
            try {
                ...
                reporter.logger.warn(
                    'Unable to get asset link "' + assetLink + '" ' + e.stack,
                );
                res.status(500).end(e.message);
            }
        });
    ...
    reporter.beforeRenderListeners.insert(
        ...
        this,
        async (req) => {
            const sharedHelpersAssets = await reporter.documentStore
                .collection("assets")
                .find({ isSharedHelper: true }, req);

            if (
                sharedHelpersAssets.length > 0 &&
                typeof req.template.helpers === "object"
            ) {
                reporter.logger.warn(
                    "Cannot add shared helpers when passing helpers as object",
                    req,
                );
            } else {
                const assetContents: any[] = await Promise.all(
                    sharedHelpersAssets.map((a) =>
                        readAsset(
                            reporter,
                            definition,
                            a._id,
            ...
            req.template.content = await evaluateAssets(
                reporter,
                definition,
            ...
            if (
                req.template.helpers &&
                typeof req.template.helpers === "string"
            ) {
                req.template.helpers = await evaluateAssets(
                    reporter,
                    definition,
                    req.template.helpers,
                    req,
    ...
    reporter.afterTemplatingEnginesExecutedListeners.add(
        "assets",
        async (req, res) => {
            const result = await evaluateAssets(
                reporter,
                definition,
    ...
    reporter.initializeListeners.add("assets", () => {
            ...
            reporter.express.exposeOptionsToApi(definition.name, {
                ...
                officePreview: {
                    enabled:
                        reporter.options.office != null &&
                        reporter.options.office.preview != null
                            ? reporter.options.office.preview.enabled
                            : undefined,
                    showWarning:
                        reporter.options.office != null &&
                        reporter.options.office.preview != null
                            ? reporter.options.office.preview.showWarning
                            : undefined,
                },
            });
        }
        ...
        reporter.documentStore.addFileExtensionResolver(
            (doc, entitySetName, entityType, propertyType) => {
                if (
                    entitySetName === "assets" &&
                    propertyType.document.content
                ) {
        ...
        if (reporter.beforeScriptListeners) {
            reporter.beforeScriptListeners.add("assets", (scriptDef, req) => {
                return evaluateAssets(
                    reporter,
                    definition,
                    scriptDef.script,
                    req,
        ...
        reporter.documentStore
            .collection("assets")
            .beforeInsertListeners.add("assets", async (entity) => {
                const allowAssetsLinkedToFiles =
                    definition.options.allowAssetsLinkedToFiles !== false;
                ...
                if (entity.link) {
                    if (!allowAssetsLinkedToFiles) {
                        throw reporter.createError(
                            'Can\'t set .link in asset when "allowAssetsLinkedToFiles" option is false',
                            {
                                statusCode: 400,
                                weak: true,
                            },
        ...
        reporter.documentStore
            .collection("assets")
            .beforeUpdateListeners.add("assets", async (query, update) => {
                const allowAssetsLinkedToFiles =
                    definition.options.allowAssetsLinkedToFiles !== false;
                ...
                if (
                    ...
                    try {
                        ...
                        throw reporter.createError(
                            `Unable to access file ${linkPath(
                                reporter,
                                definition,
                                update.$set.link,
                            )}`,
...
InitReportAssets.isAssetPathValid = isAssetPathValid;

export const ReportAssets = (options) => {
    return {
        name: "assets",
        optionsSchema: {
            extensions: {
